...
Future<String> saveTemporaryReceipt(BuildContext context, TempDraftModel tempDraftModel, String type) async {
...
//    List<SoList> dataList = solists.map((i) => SoList.fromJson(i)).toList();
    ...
    List<OutstandingARS> lists = await helper.retrieveTRInvoicesBySaved(tempDraftModel.id);

    var doc_number = tempDraftModel.receiptNo;

    List<PDCModel> flexpdclist = [];
    PDCModel pdcModel = new PDCModel(
        paymentmethod: tempDraftModel.paymentMethod,
        chequeno: tempDraftModel.chequeNo,
        paymentamount: tempDraftModel.paymentAmount,
        isrchq: "0",
        rchqdate: "",
        bankcharge: "0.00",
        tobankrate: "1.00",
    ...
    if (type.endsWith("draft")) {
      ...
      var response = await Dio().post(Api.baseUrl + "/api/v1/createTemporaryReceipt",
          queryParameters: queryParameters, data: jsonEncode(data), options: Options(headers: {"Content-Type": "application/json"}));
